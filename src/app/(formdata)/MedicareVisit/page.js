...
export default function MedicareVisit() {
  ...
  const handlePreventiveFieldChange = (id, field, value) => {
    setFormData((prev) => {
      const updatedpreventive_screening = prev.preventive_screening.map(
  ...
  const handleDeleteDynamicField = (id) => {
    setFormData((prev) => ({
      ...prev,
      preventive_screening: prev.preventive_screening.filter(
        (item) => item.id !== id
      ),
    }));
  };
  ...
  const handleDeleteImmunizationField = (id) => {
    setFormData((prev) => ({
      ...prev,
      immunizationFields: prev.immunizationFields.filter(
        (item) => item.id !== id
      ),
    }));
  };
        ...
        {formData.preventive_screening.map((field) => (
          <div key={field.id} className="w-full flex flex-wrap gap-2">
            <InputWithLabel
              ...
              value={field.service}
              onChange={(e) =>
                handlePreventiveFieldChange(field.id, "service", e.target.value)
              }
            />
            <InputWithLabel
