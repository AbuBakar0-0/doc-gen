...
export default function HealthMaintenence() {
  ...
  const handleHealthMaintenanceFieldChange = (id, field, value) => {
    setFormData((prev) => {
      const updatedHealthMaintenanceFields = prev.healthMaintenanceFields.map(
        (item) => (item.id === id ? { ...item, [field]: value } : item)
      );
      return {
        ...prev,
        healthMaintenanceFields: updatedHealthMaintenanceFields,
      };
    });
  };
  ...
  const handleDeleteHealthMaintenanceField = (id) => {
    setFormData((prev) => ({
      ...prev,
      healthMaintenanceFields: prev.healthMaintenanceFields.filter(
        (item) => item.id !== id
      ),
    }));
  };
  ...
  const handleDeleteImmunizationField = (id) => {
    setFormData((prev) => ({
      ...prev,
      immunizationFields2: prev.immunizationFields2.filter(
        (item) => item.id !== id
      ),
    }));
  };
  ...
  return (
    <div className="w-full flex flex-wrap justify-start items-start gap-2">
      <Divider title="Health Maintenance" />
        ...
        {formData.healthMaintenanceFields.map((field) => (
          <div key={field.id} className="w-full flex flex-wrap gap-2">
            <InputWithLabel
              ...
              onChange={(e) =>
                handleHealthMaintenanceFieldChange(
                  field.id,
                  "health_maintenance_topic",
                  e.target.value
                )
